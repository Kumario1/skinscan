...
| 2 — Concern efficacy | port D-023 mining (`src/recommendation/concern_labels.py` + `concern_stats.py`); semantic model labels followed by the versioned literal-policy layer; `ConcernEfficacySignal`; ladder concern cell → acne_general → pooled | p7 exact-set audit 44/50 (88%, gate ≥85%); cells for ≥300 products; "helped X% of n reviewers" evidence; weight scales with n |
