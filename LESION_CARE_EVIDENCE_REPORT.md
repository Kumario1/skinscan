...
Status: **AI research audit approved for MVP development and synthetic testing; not authorized for real-user or production use**
...
The Phase 0 research package passed a maximum-depth AI research audit for MVP development and synthetic testing. This is a research/engineering approval by a non-clinician AI, not clinical approval and not authorization for real-user, external-beta, or production use.

Consequently:

- development implementation against synthetic profiles and fixture images is authorized;
- the policy remains `production_eligible: false` and must fail closed outside development/test or when input represents a real person;
- the AI audit does not supply medical credentials, authorize diagnosis, or authorize personalized dosing, prescription, or procedure instructions;
- catalog verification is separate from the AI research audit and cannot make an inadequately evidenced product eligible.
...
The following sections describe evidence and synthetic-MVP test paths, not instructions for a person. “Proposed path” means the conservative machine-readable transformation in the MVP policy.
...
**Reproducibility exception.** These exact counts and SKU results are a provisional snapshot of the current dirty worktree. Two derived catalog inputs are absent from clean commit `94045f8`, and the committed verification overlay and concern-active file do not match the hashes below. The self-contained source snapshots remain auditable, but counts and SKU eligibility must be regenerated or their inputs pinned before they become fixed acceptance criteria.
...
This is enough catalog presence to test the melasma sunscreen path with synthetic fixtures, but it is not permission to expose that path to a person.
...
| `papule` | `partial_coverage_approved_for_synthetic_mvp` | `P188306`, benzoyl peroxide 2.5%; one exact variant. |
| `pustule` | `partial_coverage_approved_for_synthetic_mvp` | `P188306`, benzoyl peroxide 2.5%; one exact variant. |
...
| `melasma` | `partial_coverage_approved_for_synthetic_mvp` | Three exact iron-oxide SPF variants: `P456218`, `P476733`, `P481169`. |
...
## 7. MVP AI research-audit record

The maximum-depth research agent independently confirmed or rejected every item below and recorded exceptions:
...
The audit record contains the AI reviewer identity, non-clinician status, date, accepted report SHA-256, accepted policy version, decision, and exceptions. Rejected or unreviewed paths remain disabled.

## 8. Conservative MVP decisions for unresolved questions

- Nursing or unknown pregnancy status defers retail retinoids.
- Salicylic-acid exposure ambiguity stays unfilled; no rinse-off/leave-on inference is made.
- The single benzoyl-peroxide candidate may be exercised for papule and pustule fixtures without claiming image-assessed severity.
- Referral urgency uses explicit intake/red-flag answers, never detector confidence or inferred lesion severity.
- Probable melasma may exercise photoprotection fixtures but never reassures against malignancy; changing or symptomatic pigment findings refer.
- Raised-scar silicone remains disabled because confirmation and product coverage are insufficient.
- Follow-up wording stays within the source-stated ranges already recorded in the claim table.
- Procedure and irritation cautions remain clinician-only discussion content, with no synthetic path converted into a real-user instruction.
...
| Maximum-depth AI research audit recorded | Pass — development/synthetic scope only |
| MVP development implementation authorized | Yes — synthetic profiles and fixture images only |
| Real-user, external-beta, or production use authorized | **No — fail closed** |

The next authorized step is implementation and testing with synthetic profiles and fixture images; any runtime translation must be `test_only: true` under test fixtures. Any external release still requires all five locked D-029 gates: qualified clinician approval, an adequate frozen calibration cohort, a representative external clinician-reviewed set, authoritative timestamped verification of real catalog SKUs, and immutable remote detector identity.
