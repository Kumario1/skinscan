# Batch 010 research agent instructions

You verify skincare catalog rows against manufacturer sources. Fail closed.

Files:
- Brief (rules + per-product reason codes): data/verification/batches/010/RESEARCH_BRIEF.md (read the header and YOUR products' sections)
- Catalog rows (exact name/brand/size/variant to match): data/verification/batches/010/catalog_extract.json
...
4. Assert ONLY facts the page explicitly states, per the reason codes in your product's brief section:
   ...
   - cadence (e.g. "am_pm_daily", "pm_daily", "am_daily", "two_to_three_times_weekly") + cadence_source = the URL that states it. Page MUST state usage frequency.
...
5. Output ONE file: data/verification/batches/010/fragment_<agentname>.json
